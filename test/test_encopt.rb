...
class TestEncodeOption < Test::Unit::TestCase
  ...
  test ":color_type" do |type|
    enc = assert_nothing_raised {
      PNG::Encoder.new(128, 133, :color_type => type)
    }
  ...
  test "bad :color_type" do |arg|
    assert_raise_kind_of(arg[1]) {
      PNG::Encoder.new(128, 133, :color_type => arg[0])
    }
  end
  ...
  test "true :interlace" do |val|
    enc = assert_nothing_raised {
      PNG::Encoder.new(128, 133, :color_type => :RGBA, :interlace => val)
    }
  ...
  test ":compression" do |val|
    enc = assert_nothing_raised {
      PNG::Encoder.new(128, 133, :color_type => :RGBA, :compression => val)
    }
  ...
  test ":text" do |val|
    enc = assert_nothing_raised {
      PNG::Encoder.new(128, 133, :color_type => :RGBA, :text => val)
    }
